...
### 45px版(フルカラーのまま表示される側)は別ソース+円形マスクが必要

45pxは着色されない代わりに、**バニラ/他Modの指導者と並んだときに見た目が揃っている必要がある**(文明選択画面のドロップダウン等、公式文明・他Mod文明のバッジと横に並ぶ)。ここで2つハマりどころがある:

- **白シルエット版と同じ元画像を流用しない**: 白シルエット化は「白を透明に、色を不透明に」という変換なので、45pxのようなフルカラー表示にはそのまま使えない(45pxは変換前の見た目を保ちたいので別ソースが要る)。本リポジトリでは`ichijou-corporation-logo-circle.png`(45px専用、フルカラーのまま)と`ichijou-corporation-logo-circle-for-transparent.png`(他サイズ用、白シルエット化前提)の2ファイルに分離した(`tools/png2dds/gen-icon-sources.ts`の`iconSources`で参照元を出し分け)
- **円の外側はアルファで透明にする(GIMPで手動マスクするより、`maskToInscribedCircle`のような数学的に正確な円形マスクを使う)**: 円の外が不透明(または手動マスクの縁がガタついている)だと、他の指導者の綺麗な円形バッジと並んだときだけ「浮いて」見える(境界が squareに近い/ガタついているのが単体では気づきにくいが、並べると一目瞭然になる)。本リポジトリでは指導者ポートレートに使っている`maskToInscribedCircle`(中心からの距離で半径判定、境界は半ピクセルでフェード)を45px版にも適用して解決した
