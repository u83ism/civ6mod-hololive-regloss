...
SKILL.md 4節に書いていた「バッジアイコンはPNG直置きで動く、ModBuddy不要」は**裏取りできておらず誤りだった**。PNG直置き・DDS直置き・`.tex`手書きだけ追加、を順に試したが全部「？」フォールバックのまま、エラーログも一切出ないまま静かに失敗した。**結論: ModBuddyでの実ビルド(コンパイルして`.blp`を生成)が必須で、ローズファイル直置きのショートカットは無い。** 正しい手順は`https://civ6wiki.info/?MOD/作成方法/新文明・指導者/指導者アイコンの作り方`および`.../文明アイコン`ページに実践記録がある(`research-mod` Skill参照)。
