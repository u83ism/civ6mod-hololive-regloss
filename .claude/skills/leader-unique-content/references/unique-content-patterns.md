# 固有ユニット/区域/施設/建造物チュートリアル要約(civ6wiki.info)
...
6. **アイコン**: 既存の近い要素のIconDefinitionsをコピーして`Name`だけ書き換える使い回しが手軽。専用アイコンを新規に作る場合は`leader-icons` Skillの`references/icon-blp-pipeline.md`のBLPパイプラインが別途必要
