...
- リーダー選択画面(ゲーム設定画面)の全身ポートレート「Leader Placard」を実装(ローディング画面用ポートレート・背景テクスチャを流用。設定前は実体の無いダミーテクスチャ名を指していたため空表示だったことが判明。未実機確認)
...
- [ ] 外交交渉画面の背景(`DiplomacyInfo`)・リーダー選択画面の全身ポートレート(`Players.Portrait`/`PortraitBackground`)実装の実機確認
